...
function App() {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (cData.length > 0) {
          ...
          const leadsRes = await fetch(`${API_URL}/leads?course=${firstCourse.id}`);
  ...
  const handleCourseSwitch = async (course, shouldFetch = true) => {
    ...
    if (shouldFetch) {
      ...
      try {
        const res = await fetch(`${API_URL}/leads?course=${course.id}`);
  ...
  const fetchLeads = async (courseId) => {
    try {
      const res = await fetch(`${API_URL}/leads?course=${courseId || activeCourse?.id}`);
      const data = await res.json();
      setLeads(data);
  ...
  const handleDeleteCourse = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/courses/${activeCourse.id}`, { method: 'DELETE' });
      if (res.ok) {
        setCourses(prev => prev.filter(c => c.id !== activeCourse.id));
        const first = courses.find(c => c.id === 'default') || courses[0];
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (res.ok) await fetchLeads(activeCourse.id);
      else {
        const err = await res.json();
  ...
  const updateLeadStatus = async (leadId, newStatus) => {
    try {
      const res = await fetch(`${API_URL}/leads/${leadId}?course=${activeCourse.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleBulkMove = async (newStatus) => {
    ...
    try {
      const res = await fetch(`${API_URL}/leads/bulk/update?course=${activeCourse.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/leads/bulk/delete?course=${activeCourse.id}&${qs}`, { method: "DELETE" });
      if (res.ok) {
        await fetchLeads(activeCourse.id);
        setSelectedLeads(new Set());
      }
  ...
  const saveNotes = async (leadId) => {
    try {
      const res = await fetch(`${API_URL}/leads/${leadId}?course=${activeCourse.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ notes: notesDraft[leadId] || "" })
      });
      const updated = await res.json();
      setLeads(prev => prev.map(l => l.id === leadId ? updated : l));
  ...
  const deleteLead = async (leadId) => {
    ...
    try {
      await fetch(`${API_URL}/leads/${leadId}?course=${activeCourse.id}`, { method: "DELETE" });
  ...
  const deleteSale = async (saleId) => {
    ...
    try {
      await fetch(`${API_URL}/sales/${saleId}`, { method: "DELETE" });
  ...
  const handleNewSaleSubmit = async (e) => {
    ...
    try {
      const endpoint = convertingLead ? `${API_URL}/leads/${convertingLead.id}/convert?course=${activeCourse.id}` : `${API_URL}/sales`;
        ...
        {activeTab === 'pipeline' && (
          <div className="filter-section">
            ...
            <button className="primary-button" onClick={handleSearch} disabled={isSearching || !activeCourse}>
              <Search size={18} /> {isSearching ? 'Scouting Neighborhood...' : 'Scout New Prospects'}
            </button>
